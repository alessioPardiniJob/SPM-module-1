...
int main(int argc, char** argv) {
    ...
    volatile uint64_t   l_uDummy;
    
    // Vettori
    vector<uint64_t>            l_vKeys;
    vector<no_init_t<uint64_t>> l_vPart_id;
    ...
    compute_partitions(l_vKeys.data(), (uint64_t*)l_vPart_id.data(), 1000, l_uP);
    ...
    for (l_i = 0; l_i < l_iRUNS; ++l_i) {
        ...
        compute_partitions(l_vKeys.data(), (uint64_t*)l_vPart_id.data(), l_stN, l_uP);
    ...
    l_uDummy = ((uint64_t*)l_vPart_id.data())[l_stN / 2]; 
    (void)l_uDummy;
