...
int main(int argc, char** argv) {
    ...
    cout << "========================================================\n";
    cout << "           BENCHMARK EXECUTION (N = 50M)              \n";
    cout << "========================================================\n";
    ...
    if ((reinterpret_cast<uintptr_t>(l_vKeys.data()) % 32 != 0) || 
        ...
        cout << "[!] WARNING: Memory is not 32-byte aligned!\n";
    }

    // ==========================================
    // NEW: LOOP FOR SWEEP OF P (2, 16, 256, 4096, 65536)
    // ==========================================
    vector<uint32_t> p_values = {2, 16, 256, 4096, 65536};

    for (uint32_t l_uP : p_values) {
        cout << "\n\n========================================================\n";
        cout << " >>> TEST SWEEP P: " << l_uP << " PARTITIONS <<<\n";
        cout << "========================================================\n";

        // ==========================================
        // TEST A: INTRINSICS (CPU)
        // ==========================================
        cout << "\n[--- 1. CPU EXECUTION (INTRINSICS) ---]\n";
        ...
        for (l_i = 0; l_i < l_iRUNS; ++l_i) {
            ...
            // nvcc defines __CUDACC__, so the macro creates timeKernel_CPU already in ms
        ...
        cout << "   - Throughput  : " << l_dThroughput << " Mkeys/s\n";
        ...
        cout << "   - Median (Kernel-only): " << fixed << setprecision(2) << l_dMedian_ms << " ms\n";
        cout << "   - Std Dev (Kernel-only): " << l_dStd_dev << " ms\n";
        cout << "   - Throughput   (Kernel-only): " << l_dThroughput << " Mkeys/s\n";
        cout << "   [Last Run Breakdown] H2D: " << t_h2d << " ms | Kernel: " << t_kernel << " ms | D2H: " << t_d2h << " ms\n";
