                        ...
                        uint32_t p_uP)
{
    ...
    // 1. PROLOGUE
    // Process elements until the destination pointer is 32-byte aligned
    ...
    for (; l_stI < l_stLimit; l_stI += 16) {
        ...
        // FINAL TOUCH: Software prefetch
        // Prefetch 128 bytes ahead to reduce cache misses
        _mm_prefetch((const char*)&p_pKeys[l_stI + 128], _MM_HINT_NTA);

        // Load 16 keys into 4 YMM registers
        ...
        // Float-domain bypass for 32-bit block shuffling
        ...
        // Intra-lane alignment and apply partition mask
        ...
        // Pack to 16-bit and correct lane order
        __m256i packed_16 = _mm256_permute4x64_epi64(_mm256_packus_epi32(p1, p2), 0xD8);

        // Streaming store to avoid polluting cache
